...
import { Users, UserCheck, Activity, Search, CheckCircle, Clock, Link as LinkIcon } from 'lucide-react';
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
        ...
        <StatCard title="Assigned to Admin" value={stats.assignedUsers} icon={LinkIcon} color="text-indigo-400" />
      ...
      <div className="bg-sr-panel border border-sr-gold/20 rounded-xl shadow-2xl overflow-hidden">
                  ...
                  <tr key={user.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`text-xs font-bold px-3 py-1 rounded border border-gray-600 ${
                            user.role === 'ADMIN' ? 'bg-purple-900/30 text-purple-300 border-purple-500/30' : 'bg-black/40 text-gray-300'
                        }`}>{user.role}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {/* --- FIX: Added specific border color classes back --- */}
        ...
        <div className="bg-black/20 px-4 py-3 flex items-center justify-between border-t border-sr-gold/10 sm:px-6">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="text-xs font-bold text-sr-gold hover:text-white disabled:opacity-30 uppercase tracking-wider">Previous</button>
          <span className="text-xs text-gray-500">Page {page} of {totalPages}</span>
          <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="text-xs font-bold text-sr-gold hover:text-white disabled:opacity-30 uppercase tracking-wider">Next</button>
        </div>
      </div>
    </>
  );
