...
import { Users, Search, ShieldCheck, LogOut } from 'lucide-react';

const AdminPage = () => {
  // Always start as an empty array []
  ...
  useEffect(() => {
    const fetchMyUsers = async () => {
      try {
        ...
        // --- FIX 1: Safety Check ---
        // Only set users if the response is actually an array (List)
        if (Array.isArray(res.data)) {
            setUsers(res.data);
        } else {
            console.error("API Error: Expected list but got:", res.data);
            setUsers([]); // Fallback to empty list so app doesn't crash
        }

      } catch (err) {
        console.error("Access Error", err);
        // If 401/403 (Unauthorized), kick them out
  ...
  return (
    <div className="min-h-screen bg-sr-blue font-sans text-gray-100">
      ...
      <nav className="bg-sr-panel border-b border-sr-gold/20 px-8 py-4 flex justify-between items-center shadow-lg">
        ...
        <button onClick={handleLogout} className="text-gray-400 hover:text-white flex items-center gap-2 transition">
        ...
        <div className="bg-sr-panel border border-white/10 rounded-xl overflow-hidden shadow-2xl">
            ...
            <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                    {/* --- FIX 2: Check Array.isArray again before mapping --- */}
